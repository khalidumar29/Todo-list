...
const TodoDetailsModal = ({ id }) => {
  ...
  const handleSubmit = (e) => {
    ...
    fetch(`https://moont-tech-khalidumar29.vercel.app/todo/${id}`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`, // assuming you have a JWT token
      },
      body: JSON.stringify(updatedTodo),
    })
      .then((response) => response.json())
      .then((data) => {
        data.ok && toast.success(`${todo.title} successfully update!!`);
